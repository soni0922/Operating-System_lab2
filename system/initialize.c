...
static	void	sysinit()
{
	...
	for(i = 0; i < NPIPE; i++){
	       pipetab[i].pipe_state=PIPE_FREE;
	       //kprintf("\npipe no: %d and pipe state: %d,owner: %d, reader: %d, writer: %d\n",i,pipetab[i].pipe_state,pipetab[i].pipe_owner,pipetab[i].pipe_reader_id,pipetab[i].pipe_writer_id);
	}
